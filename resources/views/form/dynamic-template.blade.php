<fieldset {{ $attributes->merge(['class' => 'dynamic-fieldset']) }} data-min="{{ $min }}" data-max="{{ $max }}">
    ...
    <div class="dynamic-list">
      ...
      @forelse ($items as $item)
        <div class="d-flex align-items-end dynamic-item dynamic-item-current">
            ...
            @if($sortable)
              <button class="btn drag {{ $btnMove['class'] }}" aria-label="{{ $btnMove['label'] }}">
                {!! $btnMove['value'] !!}
              </button>
            ...
            <div class="dynamic-item-btn ml-auto" >
              <input type="checkbox" class="btn-check dynamic-delete-checkbox" id="btn-check-{{ $item->id }}" name="{{ $name.'_to_delete[]' }}" value="{{ $item->id }}" aria-label="{{ $btnDelete['label'] }}" autocomplete="off" >
              <label class="btn dynamic-delete {{ $btnDelete['class'] }}" for="btn-check-{{ $item->id }}">{!! $btnDelete['value'] !!}</label>
            </div>
      ...
      @foreach (old($name.'_to_add',[]) as $oldKey => $value)
        @if(Str::endsWith($oldKey, '_'.$type))
          <div class="d-flex align-items-start dynamic-item dynamic-item-old">
            ...
            <div class="dynamic-item-btn">
              <button type="button" class="btn dynamic-remove {{ $btnRemove['class'] }}" aria-label="{{ $btnRemove['label'] }}">{!! $btnRemove['value'] !!}</button>
            </div>
    ...
    <div class="d-flex align-items-enter">
        ...
        @endif
        <button type="button" class="ml-auto btn dynamic-add {{ $isDynamic() ? '' : 'd-none' }} {{ $btnAdd['class'] }}" aria-label="{{ $btnAdd['label'] }}">{!! $btnAdd['value'] !!}</button>
    </div>
